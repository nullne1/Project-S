...
function Worm.new(name : string, speed : number, spawnCFrame : CFrame, farm : Model) : table
...
function Worm.goToLeaf(wormBody : Part, farm : Model) : nil
	...
	local leafGoal = {
		...
		CFrame = (wormBody.CFrame * CFrame.Angles(0, 0, math.rad(90)))
	}
    ...
    -- start pupating
	local cocoon = CocoonModule.new(wormBody, wormBody.CFrame)
	cocoon:start()
